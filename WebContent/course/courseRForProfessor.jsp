...
<head>
	<meta charset="UTF-8">
	<meta name="description" content="courseRForProfessor.jsp">
				...
				<div class="col-md-2">
		            <div class="panel status panel-danger">
		                <div class="panel-heading">
		                    ...
		                    <c:if test="${dsProfessorregList[0].COURSE_TOTAL_SCORE != null}">${dsProfessorregList[0].COURSE_TOTAL_SCORE}</c:if>
		                    <c:if test="${dsProfessorregList[0].COURSE_TOTAL_SCORE == null}">0</c:if>
		                    </h1>
		                </div>
			                    	...
			                    	<c:forEach var="idx" begin="1" end="9">
			                         <tr>
	                            		...
	                            		<td style="text-align: center; vertical-align: middle;">
		                            		<c:forEach items="${dsProfessorregList}" var="dsProfessorregList">
		                            			<c:if test="${dsProfessorregList.COURSE_DAY == '월' && dsProfessorregList.COURSE_PERIOD == idx}">${dsProfessorregList.COURSE_NAME}<br>${dsProfessorregList.COURSE_CLASSROOM}</c:if>
		                            		</c:forEach>
	                            		</td>
	                            		<td style="text-align: center; vertical-align: middle;">
	                            			<c:forEach items="${dsProfessorregList}" var="dsProfessorregList">
		                            			<c:if test="${dsProfessorregList.COURSE_DAY == '화' && dsProfessorregList.COURSE_PERIOD == idx}">${dsProfessorregList.COURSE_NAME}<br>${dsProfessorregList.COURSE_CLASSROOM}</c:if>
		                            		</c:forEach>
	                            		</td>
	                            		<td style="text-align: center; vertical-align: middle;">
	                            			<c:forEach items="${dsProfessorregList}" var="dsProfessorregList">
		                            			<c:if test="${dsProfessorregList.COURSE_DAY == '수' && dsProfessorregList.COURSE_PERIOD == idx}">${dsProfessorregList.COURSE_NAME}<br>${dsProfessorregList.COURSE_CLASSROOM}</c:if>
		                            		</c:forEach>
	                            		</td>
	                            		<td style="text-align: center; vertical-align: middle;">
	                            			<c:forEach items="${dsProfessorregList}" var="dsProfessorregList">
		                            			<c:if test="${dsProfessorregList.COURSE_DAY == '목' && dsProfessorregList.COURSE_PERIOD == idx}">${dsProfessorregList.COURSE_NAME}<br>${dsProfessorregList.COURSE_CLASSROOM}</c:if>
		                            		</c:forEach>
	                            		</td>
	                            		<td style="text-align: center; vertical-align: middle;">
	                            			<c:forEach items="${dsProfessorregList}" var="dsProfessorregList">
		                            			<c:if test="${dsProfessorregList.COURSE_DAY == '금' && dsProfessorregList.COURSE_PERIOD == idx}">${dsProfessorregList.COURSE_NAME}<br>${dsProfessorregList.COURSE_CLASSROOM}</c:if>
		                            		</c:forEach>
	                            		</td>
			                         </tr>
			                        </c:forEach>
